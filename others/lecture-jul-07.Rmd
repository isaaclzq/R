...
```{r, eval=FALSE}
# function convert
convert1 <- function(x, to = "celsius") {
  to = tolower(to)
  return (convert(x, to))
}
...
for (value in c("a", "e", "i", "o", "u")){
  print(sprintf("- %s: %d", value, sum(value == lets)))
}
```
...
# your script
count = function(x){
  num_letters = length(x)
  num_vawels = 0
  for (value in c("a", "e", "i", "o", "u")){
    num_vawels = num_vawels + sum(value == lets)
  }
  print(sprintf("letters: %d", num_letters))
  print(sprintf("vowels: %d", num_vawels))
  print(sprintf("consonants: %d", num_letters - num_vawels))
}
count1 = function(){
  return (count(lets))
}
